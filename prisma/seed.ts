...
async function main() {
  ...
  // Create badges (using individual creates to avoid skipDuplicates issue)
  const badgeData = [
    { userId: user1.id, type: 'MAKER' as const },
    { userId: user1.id, type: 'REVENUE_100K' as const },
    { userId: user2.id, type: 'MAKER' as const },
    { userId: user2.id, type: 'TOP_GUESSER' as const },
    { userId: user3.id, type: 'MAKER' as const },
    { userId: user3.id, type: 'EARLY_ADOPTER' as const },
  ];
...
## Why we built this
We were frustrated with complex form builders that required developer involvement. FormFlow makes it easy for anyone to create professional forms.`,
...
Built for freelancers who hate admin work.`,
  ...
  const commentData = [
    {
      content: 'Love the clean UI! How long did it take to build the MVP?',
      userId: user2.id,
      startupId: startup1.id,
    },
    {
      content: 'Been using this for 3 months. Game changer for our team!',
      userId: user3.id,
      startupId: startup1.id,
    },
    {
      content: 'Great product! What tech stack are you using?',
      userId: user1.id,
      startupId: startup2.id,
    },
    {
      content: 'Interested in the acquisition. DMed you!',
      userId: user1.id,
  ...
  // Create some guesses
  const guessData = [
    { userId: user2.id, startupId: startup1.id, range: 'RANGE_10K_20K' as const, isCorrect: true },
    { userId: user3.id, startupId: startup1.id, range: 'RANGE_5K_10K' as const, isCorrect: false },
    { userId: user1.id, startupId: startup2.id, range: 'RANGE_1K_5K' as const, isCorrect: true },
    { userId: user1.id, startupId: startup3.id, range: 'RANGE_5K_10K' as const, isCorrect: true },
  ];
